...
Dump of assembler code for function main:
   ...
   0x08048532 <+9>:	sub    esp,0xa0                                 ; 160 bytes local variable
   ...
   0x0804855a <+49>:	rep stos DWORD PTR es:[edi],eax             ; memset esp+0x50 to 0x0 on 19 * 4 (76) bytes
   0x0804855c <+51>:	mov    eax,DWORD PTR [ebp+0xc]              ; load argv
   0x0804855f <+54>:	add    eax,0x4                              ; load argv[1]
   ...
   0x08048577 <+78>:	call   0x80483c0 <strncpy@plt>              ; strncpy(esp+0x50, argv[1], 40)
   0x0804857c <+83>:	mov    eax,DWORD PTR [ebp+0xc]
   0x0804857f <+86>:	add    eax,0x8                              ; load argv[2]
   ...
   0x0804859a <+113>:	call   0x80483c0 <strncpy@plt>              ; strncpy(esp+0x50+0x28, argv[2], 32)
   0x0804859f <+118>:	mov    DWORD PTR [esp],0x8048738            ; "LANG"
   0x080485a6 <+125>:	call   0x8048380 <getenv@plt>               ; load environment variable name "LANG"
   0x080485ab <+130>:	mov    DWORD PTR [esp+0x9c],eax             ; store result to esp+0x9c
   0x080485b2 <+137>:	cmp    DWORD PTR [esp+0x9c],0x0             ; if nil
   0x080485ba <+145>:	je     0x8048618 <main+239>                 ; greetuser
   ...
   0x080485d6 <+173>:	call   0x8048360 <memcmp@plt>               ; memcpy(esp+0x9c, "fi", 2)
   0x080485db <+178>:	test   eax,eax
   0x080485dd <+180>:	jne    0x80485eb <main+194>                 ; if nil
   0x080485df <+182>:	mov    DWORD PTR ds:0x8049988,0x1           ; <language> *0x8049988 = 1
   ...
   0x08048605 <+220>:	call   0x8048360 <memcmp@plt>               ; memcpy(esp+0x9c, "nl", 2)
   0x0804860a <+225>:	test   eax,eax
   0x0804860c <+227>:	jne    0x8048618 <main+239>                 ; if nil
   0x0804860e <+229>:	mov    DWORD PTR ds:0x8049988,0x2           ; <language> *0x8049988 = 2
   ...
   0x08048629 <+256>:	rep movs DWORD PTR es:[edi],DWORD PTR ds:[esi] ; cpy 19 bytes from esi to edi
...
Dump of assembler code for function greetuser:
   ...
   0x0804848a <+6>:	mov    eax,ds:0x8049988                 ; load <language>
   0x0804848f <+11>:	cmp    eax,0x1                      ; if == 1 (fi)
   0x08048492 <+14>:	je     0x80484ba <greetuser+54>
   0x08048494 <+16>:	cmp    eax,0x2                      ; if == 2 (nl)
   0x08048497 <+19>:	je     0x80484e9 <greetuser+101>
   0x08048499 <+21>:	test   eax,eax                      ; if nil
   0x0804849b <+23>:	jne    0x804850a <greetuser+134>
   0x0804849d <+25>:	mov    edx,0x8048710                ; "Hello "
   0x080484a2 <+30>:	lea    eax,[ebp-0x48]               ; copy to ebp-0x48
   0x080484a5 <+33>:	mov    ecx,DWORD PTR [edx]
   0x080484a7 <+35>:	mov    DWORD PTR [eax],ecx          ; copy 4 bytes
   0x080484a9 <+37>:	movzx  ecx,WORD PTR [edx+0x4]
   0x080484ad <+41>:	mov    WORD PTR [eax+0x4],cx        ; copy 2 bytes
   0x080484b1 <+45>:	movzx  edx,BYTE PTR [edx+0x6]       ; copy 1 byte
   0x080484b5 <+49>:	mov    BYTE PTR [eax+0x6],dl
   0x080484b8 <+52>:	jmp    0x804850a <greetuser+134>
   0x080484ba <+54>:	mov    edx,0x8048717                ; block when LANG=nl - load string "Hyvää päivää "
   0x080484bf <+59>:	lea    eax,[ebp-0x48]               ; copy to ebp-0x48
   ...
   0x080484e9 <+101>:	mov    edx,0x804872a                ; block when LANG=nl - load string "Goedemiddag! "
   0x080484ee <+106>:	lea    eax,[ebp-0x48]               ; copy to ebp-0x48
   ...
   0x08048509 <+133>:	nop
   0x0804850a <+134>:	lea    eax,[ebp+0x8]                ; load first parameter
   0x0804850d <+137>:	mov    DWORD PTR [esp+0x4],eax
   0x08048511 <+141>:	lea    eax,[ebp-0x48]
   0x08048514 <+144>:	mov    DWORD PTR [esp],eax
   0x08048517 <+147>:	call   0x8048370 <strcat@plt>       ; strcat(greeting string, first parameter)
   0x0804851c <+152>:	lea    eax,[ebp-0x48]
   0x0804851f <+155>:	mov    DWORD PTR [esp],eax
   0x08048522 <+158>:	call   0x8048390 <puts@plt>         ; puts(greeting string)
